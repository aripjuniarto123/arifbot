...
//SUBSCRIBE VEYA GANZ,TO 100K SUBS ATAU LEBIH
...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=13479498931:+23479498931\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
            + 'END:VCARD' // jangan di ubah
//SETINGAN
apikey = 'Asyapikeycanz' 
prefix = '!' 
owner = '13479498931' 
cr = 'LoL-Human Team'
//Setingan\\
...
async function starts() {
 ...
 if (isCmd) cmdadd()
		...
		case 'daftar':
               ...
               asyacanz = fs.readFileSync(`lib/asyacanz.jpg`)
                veri = sender
                if (isGroup) {
                    addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    await lolteam.sendMessage(from, asyacanz, image, {quoted: lol, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
    ...
    case 'ttp':
		...
		case 'tebakbendera':
			...
			if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK cowner')
				...
				case 'namaninja': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: Asyacanz`)
